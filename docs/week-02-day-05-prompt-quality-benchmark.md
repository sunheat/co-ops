...
| 减少幻觉代理指标 | `grounded` 同时要求答案含预期关键结论、必要引用均有效、答案不含该题预置的矛盾结论。矛盾结论按完整字词、数值或日期/时间 token 匹配，并忽略紧邻的否定表达，避免把 `120` 中的 `20` 或 “not USD 20” 误判为错误答案。事实性输入（如锁定开始时间）不会被配置为矛盾结论。它是固定题集的可复核代理指标，不能证明模型在任意任务中都不会幻觉。 |
